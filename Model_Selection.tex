...
\subsection{Estimating evidences using SMC$^2$}


Sequential Monte Carlo (SMC) algorithms use a weighted collection of particles to approximate distributions. By updating the weights and the particles as observations arise, we can
...
A series of intermediary distributions $\{ \pi_t \}_{t=1,...,T}$ are chosen that `close-in' on the target distribution, $\pi_T$, from some easily sampled distribution $\pi_1$. Usually this sequence of distributions is taken to be the sequence formed by adding the data a point at a time.
...
One of the earliest and well known implementations of SMC \cite{Gordon1993} is a collection of algorithms designed to sample from the sequence of filtering distributions $\pi_t = \pi(X_t \vert \theta, Y_{1:t})$,  and the SMC algorithms are termed particle filters (PFs) in this case.
...
\item {\bf SMC$^2$ 3.4.2.} Sample $\left( \theta, X_{1:t} \right)^{**} \sim K \left(\cdot \vert \left( \theta^j, X_{1:t}^{n,j} \right)^* \right)$ where $K$ is a PMCMC kernel that leaves $\pi (\theta, X_{1:t} \vert Y_{1:t})$ invariant.
...
A further difficulty arises because for the models of interest, the transition densities $\pi(X_{i+1} \vert X_i, \theta_m, \mathcal{M}_m)$ are not available in closed form, ruling out many approaches a priori. 
