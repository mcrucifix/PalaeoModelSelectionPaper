...
to favouring simpler phenomenological (consistent with, but not derived from the physical theory of a system) models.  The three models we consider (detailed in \S), each
...
The aim of this paper is to demonstrate that an empirical evaluation of various candidate dynamical models and
to assess the importance of the various orbital characteristics to the
glacial--interglacial cycle.
A common viewpoint is that the  information contained in a single record do not
...
likelihood.  A powerful tool for problems with intractable likelihoods is the particle
...
\label{Sec:Data}
 ...
 Attempts to understand the dynamical behaviour of the palaeoclimate typically involve four components: data consisting of palaeoclimate records;  models of the climate;  drivers/forcings of the climate (such as CO$_2$ emissions, or more pertinently for palaeoclimate, the solar forcing), and  a statistical model relating the three previous components.

The aim of this paper  is  to demonstrate  the statistical methodology necessary for combining these components in order to answer the questions we imagine palaeoclimate scientists may wish to ask. That is to say, given some data and a selection of models, we show how to fit these models, and to assess which model is best supported by the data. Scientific aspects of the approach can (and we hope will) be improved upon: different datasets, richer models, and more accurate statistical modelling may all follow. Here, our aim is merely to show that the methodology and computer power is now of sufficiently advanced state that the Bayesian inference problems can be answered.

\rdw{Michel - I was aiming to describe the holy trinity diagram you used in Uccle (the triangle of approaches) with the aim of unifying some of them using stats. Its kind of an apologia to stress that this is a proof of concept of the stats. I'm sure it can be improved.}
...
\rdw{This needs an explanatory statement - why is this important?}
...
The other part of the statistical specification is a model discrepancy...
The statistical model relating model to data consists of two parts. The first relates 

A key assumption in out statistical framework, which is needed to make the problem tractable, is that there is a conditional independence assumption that allows the likelihood to be decomposed as

\begin{eqnarray}
 \pi(X_{1:T} \vert \theta_m, \mathcal{M}_m) \pi(Y_{1:T} \vert X_{1:T}, \theta_m, \mathcal{M}_m)
 \propto  \\
 \pi(\theta_m \vert \mathcal{M}_m) \pi(X_1 \vert \theta_m, \mathcal{M}_m) \prod_{i=1}^{T-1} \pi(X_{i+1} \vert X_i, \theta_m, \mathcal{M}_m) \prod_{i=1}^T \pi(Y_i \vert X_i, \theta_m, \mathcal{M}_m).\nonumber
\end{eqnarray}
