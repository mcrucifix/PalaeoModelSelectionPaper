...
In total we use 5 models: SM91 (forced and unforced, with 14 and 11 parameters respectively), T06 (forced and unforced, with 13 and 10 parameters respectively), and PP12 (forced with 16 parameters).
We do not consider an unforced PP12 model as the deglatiation-glaciation transition depends only on the astronomical forcing.
This is not the case for SM91 and T06, which both oscillate in the absence of any external forcing.
The priors used for each model are summarised in Table~\ref{Tab:Priors}.

The model evidence for each model is estimated by SMC$^2$ and summarised in Table~\ref{Tab:Evidence}.
The Bayes factor of two models is given by the ratio of the Model Evidence estimates.
A common interpretation of Bayes factors suggests that a Bayes factor of 3 is worth mentioning, and that a Bayes factor of 20 is a strong indication that one model is supported over another \cite{Kass1995}.
